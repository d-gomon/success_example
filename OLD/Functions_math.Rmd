...
riskcalc <- function(dat, coefficients){
  #Calculate risk for dataset dat according to Cox PH model using coefficients.
  #Covariates should be included in dat as any variables which are not Si, Xi or Ti.
  return(c(exp(as.matrix(dat[, !names(dat) %in% c("Si", "Xi", "Ti")]) %*% coefficients)))
}

```

```{r}
gensurvtime <- function(invchaz, dat, coefficients, mu = log(1) ){
  ...
  #Coefficients should be a p-vector containing the coefficients
  ...
  U <- runif(nrow(dat))
  Times <- invchaz(-log(U) * exp(-mu)* exp(- as.matrix(dat[, !colnames(dat) %in% c("Si", "Xi", "Ti")]) %*% coefficients))
  return(Times)
}

gendatapoisswcov <- function(psi, t, coeffgen, invchaz, mugen = log(1)){
  ...
  datn$Xi <- as.vector(ceiling(gensurvtime(invchaz = invchaz, dat = datn, coefficients =  coeffgen, mu = mugen)))
  datn$Ti <- datn$Si + datn$Xi
  return(datn)
}
gendatapoiss <- function(psi , t, coeffgen, invchaz, mugen = log(1)){
  ...
  arrivals <- round(poissarriv(psi = psi, t = t))
  n <- length(arrivals)
  datn <- data.frame(asd = rep(0, n), Si =  arrivals )
  datn$Xi <- as.vector(ceiling(gensurvtime(invchaz = invchaz, dat = datn, coefficients =  coeffgen, mu = mugen)))
  datn$Ti <- datn$Si + datn$Xi
  return(datn)
}
...
```{r}
#Riskcalcv2 is more general, as you can supply a coxmod such as a coxph object.
#Returns NA when coefficient does not contain all covariates found in formula
riskcalcv2 <- function(dat, coxmod = NULL){
...
gensurvtimev2 <- function(invchaz, dat, coxmod = NULL, mu = log(1) ){
  #This function generates Survival times for the dataset dat using the given coefficients and 
  #with rate exp(mu) times the null-rate invchaz
  #
  #invchaz should be a vectorized function returning the null-chazard at time t (one input only)
  ...
  #hazard differs from the null-rate
  U <- runif(nrow(dat))
  Times <- invchaz(-log(U) * exp(-mu)* (1/riskcalcv2(dat, coxmod)))
  return(Times)
}
#use coxmod <- list(formula = as.formula(Xi ~ age + BP + gender + BMI + unif), coefficients = c(0.001, 0.005, 0.01, 0.002, 0.02))
gendatapoisswcovv2 <- function(psi, t, coxmod, invchaz, mugen = log(1)){
  #Create a dataset with covariates age, BP, gender and unif with poisson(psi) arrivals until time t. 
  #Coeffgen are the coefficients to be used for risk-adjustment and mu the exp(mu) adjusted rate factor
  #
  #coefficients should be a p-vector with the coefficients corresponding to the null-hypothesis.
  ...
  n <- length(arrivals)
  datn <- data.frame(age = rnorm(n, 45, 17), BP = rnorm(n, 105, 12), gender = rbinom(n, 1, 0.5) ,BMI = rnorm(n, 21, 9), unif = runif(n, 0, 100), Si = arrivals)
  datn$Xi <- as.vector(ceiling(gensurvtimev2(invchaz = invchaz, dat = datn, coxmod =  coxmod, mu = mugen)))
  datn$Ti <- datn$Si + datn$Xi
  return(datn)
}
gendatapoissv2 <- function(psi , t, coxmod = NULL, invchaz, mugen = log(1)){
  #Create a dataset without covariates with poisson(psi) arrivals until time t. 
  #coxmod must either be a COXPH model or a list containing $formula and named vector $coefficients
  #
  #coefficients should be a p-vector with the coefficients corresponding to the null-hypothesis.
...
```{r}
CTGLRsim <- function(dat, coefficients, cbaseh, n = 1461, h = NULL){
  #This function fits the CTGLR chart on the given data, using the given coefficients for risk adjustment
  ...
  riskdat <- riskcalc(dat, coefficients)
...
```{r}
CTCUSUMsim <- function(dat, coefficients, cbaseh, theta, n = 1461, h = NULL){
  ...
  #cbaseh must be a function which returns the null cumulative baseline hazard at every timepoint (or at least on discrete times), if the only times of interest are the fitted ones, use basehaz(phmodel) - vectorize it!!
  ...
  revisions[as.integer(names(table(dat$Ti, exclude = Inf)))+1] <- as.integer(table(dat$Ti, exclude = Inf))
...
CTMAXGLRsim <- function(dat, coefficients, cbaseh, n = 1461, h = NULL){
  library(survival)
  stopind = FALSE
  indh <- is.null(h)
  #dat must be the dataframe containing the covariates, n x p data frame or matrix
  ...
  for(j in 1:(n+1)){
    ...
    riskdat <- riskcalc(tdat2, coefficients)
...
plot.ALL <- function(chartlist, nameslist){
  ...
  g <- ggplot(data = finaldat, mapping= aes(x = t, y = value, colour = description)) + geom_line() + scale_color_manual(values = cols)
  return(g)
}
```
...
FUNNELsim <- function(glmmodel = NULL, dat, followup, conflev = c(0.95, 0.99), p0 = NULL, time = NULL){
  ...
  #dat has to be a dataframe containing at least Xi (time until failure), and additionally the covariates to RA on
  #glmmodel is the risk-adjustment model, either an object of class "glm" or $formula and $coefficients
  #followup is time until which we consider outcomes
  ...
  if(is.null(p0)){
    p0 <- length(which(dat$Xi < followup))/length(dat$Xi)
  }
  plotframe <- data.frame(instance = character(), observed = double(), expected = double(), numcases = double())
  for(j in unique(dat$instance)){
    tempdat <- subset(dat, instance == j)
    ...
    tempobs <- length(which(tempdat$Xi <= followup))
...
CUSUMsim <- function(glmmodel = NULL, dat, followup, theta = NULL, p1 = NULL, p0 = NULL, h = NULL, time = NULL){
  ...
  dat$outcome <- as.integer(dat$Xi <= followup)
